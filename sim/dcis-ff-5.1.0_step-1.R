##-----------------------------------------------------------------------------#
#' Simulation 5.1.0 - fiber features
...
# 90 runs at `batch_size` = 10, for 900 total
...
if (file.exists(gs_fname)) {
  ...
  # IMPORTANT: ignore outer CV and just do train/test split
  gridsearch_spec <- filter(gridsearch_spec, fold == 1)

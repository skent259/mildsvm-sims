...
step1_fname <- glue("mildsvm-sims-results-{sim}-{step}.rds")
...
gridsearch_spec <- 
  ...
  mutate(across(all_of(metric_to_optimize)), ~mean(.x, na.rm = TRUE)) %>% 
  ungroup()
...
eval_spec <- 
  ...
  mutate(across(time, ~sum(.x, na.rm = TRUE))) %>% 
  slice_max(order_by = across(starts_with("mean_")), n = 1, with_ties = FALSE) %>% 
  ungroup()
